...
exports.SelectSeatsPage = class SelectYourOwnSeatPage {
  constructor(page) {
    ...
    this.ticketQuantityDecrementBtn = this.inputGroupNumber.locator(
      'button[class*="decrement"]',
    );
    this.ticketQuantityIncrementBtn = this.inputGroupNumber.locator(
      'button[class*="increment"]',
    );

    this.continueBtn = this.base.locator(
      ".syos-level-selector-container__cta button",
    );

    this.priceTypesEl = this.base.locator(".syos-level-selector-price-types");
    this.allSectorTypes = this.priceTypesEl.locator(
      '[class*="item"][tabindex]',
    );
    ...
    this.bestAvailableOrdBtn = this.priceTypesEl.locator(
      '[class*="bestavailable-order"]',
    );

    this.zoneAvailableName = this.allAvailableSectorTypes.locator(".zone");

    this.backBtn = this.base.locator(
      'button[class="syos-button syos-button--back"]',
    );
    ...
    this.seatPriceLineItem = this.basketBase.locator(
      ".syos-lineitem__price .syos-price__value",
    );
  ...
  async getAllAvailableSectorTypesCount() {
    ...
    const getNotAvailableSectorTypes =
      await this.allNotAvailableSectorTypes.count();
  ...
  async getNamesOfAvailableSectorNames() {
    ...
    for (let i = 0; i < availableZonesCount; i++) {
      availableZoneNames.push(
        await getTextFromElement(this.zoneAvailableName.nth(i)),
      );
    }
